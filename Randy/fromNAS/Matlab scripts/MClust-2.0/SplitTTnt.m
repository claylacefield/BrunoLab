function SplitTTnt(TTfname, CLfname, nTrodes)
% SplitTTnt(TTfname, CLfname, nTrodes)
%
...
%---- initialize
if ~ismember(nTrodes, [1,2,4])
   error('nTrodes must be one of 1,2, or 4.');
end
...
%---- Read Header
%% H = ReadHeader(fp); % - no header in nt system
...
for iClust = 1:nClusters
   tmpfn = [TTfn 'Split' num2str(iClust,'%02d') '.dat'];
   [newTTfp{iClust},msg] = fopen(tmpfn, 'wb', 'n');
   if newTTfp{iClust} == -1; error(['SplitTT error: ' msg]); end
end

[newTTfpOUT,msg] = fopen([TTfn 'Splitout.dat'], 'wb', 'n');
...
for iSeg = 1:nSeg   
   ...
   for iClust = 1:nClusters
      ...
      if debugON
         fprintf('   %d: Length of f = %d\n', iClust, length(f));
      end
      keep(f) = 1;
      for iRec = f'
         fwrite(newTTfp{iClust}, ts(iRec), 'int64');
         fwrite(newTTfp{iClust}, param(iRec,:), 'int32');
   ...
   %--- Write nokeep file
   for iRec = find(~keep)'
      fwrite(newTTfpOUT, ts(iRec), 'int64');
      fwrite(newTTfpOUT, param(iRec,:), 'int32');
