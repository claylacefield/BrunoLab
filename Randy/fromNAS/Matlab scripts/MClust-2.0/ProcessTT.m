function ProcessTT(TTfname, CLfname, OSCategory)
% ProcessTT(TTfname, CLfname, OSCategory)
%
% Creates T files from a given TT file and a coresponding .clusters file.
% INPUT:
...
case {'suntt', 'sun tt'},
   ProcessTTsun(TTfname, CLfname);
case {'nttt', 'nt tt'},
   ProcessTTnt(TTfname, CLfname, 4);
case {'ntst', 'nt st'},
   ProcessTTnt(TTfname, CLfname, 2);
case {'ntse', 'nt se'},
   ProcessTTnt(TTfname, CLfname, 1);
end
