...
mean_ps_z = mean(ps_z, 1);
mean_ps_z = mean_ps_z(1:skip/2);
...
for i=1:n_stim
      delta_z(i,:) = Imp(i,:)-mean_ps_z;
      p_delta_z(i,:) = (Imp(i,:)-mean_ps_z)./mean_ps_z;

    
%     delta_z(i,:) = Imp(i,:)-Imp(1,:);
%     p_delta_z(i,:) = (Imp(i,:)-Imp(1,:))./Imp(1,:);
