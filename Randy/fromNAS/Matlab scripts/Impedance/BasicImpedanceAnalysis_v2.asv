...
start = SAMPLERATE*((10)/1000);
ps_end = SAMPLERATE*((145)/1000);
...
pre_stim_i = zeros(nRecs, ps_end-start+2);
pre_stim_v = zeros(nRecs, ps_end-start+2);
ps_z = zeros(n_stim, n_trials, ps_end-start+2);

I = zeros(n_stim, n_trials, N_fft+2);
V = zeros(n_stim, n_trials, N_fft+2);
Z = zeros(n_stim, n_trials, N_fft+2);
coh = zeros(n_stim, n_trials, N_fft+2);
...
%     tail_i = fread(fiid, tail, 'float32');
    skipped_v = signal_v(start-1:ps_end );
    skipped_i = signal_i(start-1:ps_end );
%     skipped_v = signal_v(x>=start  & x<= ps_end );
%     skipped_i = signal_i(x>=start  & x<= ps_end );
    %signal_v = fread(fvid, nScans, 'float32');
    %signal_i = fread(fiid, nScans, 'float32');
    stim_v = signal_v(skip-1:(skip+window) );
    stim_i = signal_i(skip-1:(skip+window) );
    if (feof(fvid)) break; end;
    if (feof(fiid)) break; end;
    figure(h);
    subplot(2,1,1);
...
freq = SAMPLERATE/2*linspace(0,1,(N_fft+2)/2);
%matlab's fft command calculates power for DC signal as well)
Imp = zeros(n_stim, (N_fft+2)/2);
ps_Z = zeros(n_stim, (N_fft+2)/2);
Coh = zeros(n_stim, (N_fft+2)/2);
I_pspec = zeros(n_stim, (N_fft+2)/2);
V_pspec = zeros(n_stim, (N_fft+2)/2);
size(Z)
for i=1:n_stim

    mean_imp = reshape(mean(Z(i,1:stim_trial(i),1:(N_fft+2)/2)),1, (N_fft+2)/2) ;
    mean_coh = reshape(mean(coh(i,1:stim_trial(i),1:(N_fft+2)/2)),1, (N_fft+2)/2) ;
    ...
    mean_ps_z = reshape(mean(ps_z(i,1:stim_trial(i),1:(N_fft+2)/2)),1,(N_fft+2)/2);
...
grid on;
 ...
 delta_z = zeros(n_stim-1, (N_fft+2)/2);
p_delta_z= zeros(n_stim-1, (N_fft+2)/2);
for i=1:n_stim
