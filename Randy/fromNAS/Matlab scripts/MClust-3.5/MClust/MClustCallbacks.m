...
%% Load Features
	case 'LoadFeaturesButton'
        ...
        % RMB, must be reset to prevent crash in CutterRedrawAxes
		clear global MClust_CurrentFeatureData
        MClust_CurrentFeatures = [-1,-1,-1];
...
%% WriteFiles
	case 'WriteFiles'
        ...
        %convert to .cluster files - RMB
        clusterIndex = ProcessClusters(MClust_FeatureTimestamps, MClust_Clusters);
        [basedn, basefn, ext] = fileparts(MClust_TTfn);
        WriteClusterIndexFile([fullfile(MClust_TTdn, MClust_TTfn), '.cut'], clusterIndex);
        cut2clustern([fullfile(MClust_TTdn, MClust_TTfn), '.cut'], [fullfile(MClust_TTdn, MClust_TTfn), '.lv']);
