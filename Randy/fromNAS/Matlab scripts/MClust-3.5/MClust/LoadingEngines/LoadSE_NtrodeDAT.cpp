/*---------------------------------
* LoadST_NtrodeDAT (adapted from LoadST_NeuralynxNT.cpp)
* MEX file
*
* input:
*    fn = file name string
...
*
* RMB Feb 2011
* Reads .dat files generated by Ntrode
*
* ADR May 2002
...
const int recSize = 140;
const int recSizeAfterTimestamp = recSize - sizeof(unsigned long);
  ...
  // reads one record and goes to the next record
{
  ...
  // ntrode record
  unsigned long qwTimeStamp, qwTimeStamp0;
  float dwParams[2];
  short snData[32];

  int j, k;
  
  fread(&qwTimeStamp0,  sizeof(char),   4, fp);
  fread(dwParams,      sizeof(char),  8, fp);
 ...
 // 
      ...
      for(j=0; j<128; j++)
		...
		if (j%4 < 1)
			/* convert big-endian (Sun) to little-endian (PC) */
			wv[j] = (double) swapbytes(snData[j / 4]);
		else
			wv[j] = 1;
...
int GetNumberOfSpikes(char* fn){
  ...
  int error = SkipHeader(fp);  
  if (error)
    mexErrMsgTxt("Not an NTRODE .dat file.");
...
void ReadTT(char* fn, int nSpikes, double *t, double *wv){
  ...
  // skip header 
  int error = SkipHeader(fp);  // flag for new NT_format TT files (0=old SUN, 1=new NT) 
  if (error) 
    mexErrMsgTxt("Not an NTRODE .dat file.");
