...
for i = 1:n
    ...
    if (~isempty(Pwfile{i}) & ~strcmp(Pwfile{i}, 'NA') & isfinite(medfilt(i)) & ~strcmp(fp(i), 'FS') & layer{i}==4)
        % if there is a PW file listed and the data was filterable...
            
        [psp, nspikes, stimuli, spon] = PolarPlots(Pwfile{i}, answers(1), answers(2), answers(3), answers(4), 350, false);
        stimuli = stimuli';
