function [new, spikeindex, PostPeaks] = RemoveSpike4(old, msPrePeak, tailScaler, thresh)
...
%     figure(10);plot(excerpt);
%     figure(1);
    zeroindex = threshold(excerpt, 0, 2);
    if isempty(zeroindex)
        zeroindex = length(excerpt);
    end
    zeroindex = zeroindex(1);
    %
    % Step 3. calculate time from peak to 0 zero crossing AND SCALE TO
    % USER'S PREFERENCE
    msPostPeak = tailScaler * (minindex + zeroindex) / SAMPLESPERMS;
