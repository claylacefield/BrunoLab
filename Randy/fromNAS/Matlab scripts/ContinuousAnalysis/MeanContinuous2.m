...
if PLOT
    ...
    % quasi latency estimation
    subplot(2,1,2);
    DECIMATEFACTOR = 8;
    d = diff(decimate(average, DECIMATEFACTOR));
    xx = linspace(0, duration, nScans / DECIMATEFACTOR - 1);
    plot(xx, d);
    upperlim = norminv(0.999, mean(d), std(d(xx < stimOnset)));
  
    hold on;
    line([0 duration], [upperlim upperlim]);
end
