function FilterSize = FindBestFilters4(filepath, duration)
...
%
% In v3, a spike is detected by the derivative crossing a
...
%
% In v4, I added a user-defined scaler, which determines the amount of time post spike to remove
% relative to the derivative-identified tail. For example, 1.0 means remove
% exactly the time defined by the derivative-based method, 1.1 means an extra 10%,
% 2.0 means double, etc.
...
answers = inputdlg({'derivative threshold (mV)', 'pre-peak time for spike deletion (ms)', 'multiple of derivative-based time to delete', 'skip factor'}, ...
    'Parameters for spike filtering', 1, ...
    {num2str(thresh), num2str(msPrePeak), num2str(tailScaler), num2str(skipFactor)});
...
while (~feof(fid))
    ...
    line(x(posstart:posend), RemoveSpike4(signal(posstart:posend), msPrePeak, tailScaler, thresh));
    ...
    subplot(4,1,4);
    %plot(x(posstart:posend), signal(posstart:posend), 'LineStyle', ':');
    [y,spikeIndex,PostPeaks] = RemoveSpike4(signal(posstart:posend), msPrePeak, tailScaler, thresh);
    plot(x(posstart:posend), y);
    xlim([x(posstart) x(posend)]);
    %ylim([-80 thresh]);
    ...
    answers = inputdlg({'threshold (mV)', 'pre-peak time for spike deletion (ms)', 'multiple of derivative-based time to delete', 'skip factor'}, ...
        'Parameters for spike filtering', 1, ...
        {num2str(thresh), num2str(msPrePeak), num2str(tailScaler), num2str(skipFactor)});
    ...
    if (isempty(answers))
        ...
        newScaler = str2num(answers{3});
        skipFactor = str2num(answers{4});
      
        if (thresh ~= newThresh || msPrePeak ~= newPre || tailScaler ~= newScaler )
...
if strcmp(button, 'Yes')
    close;
    SeparateSpikesAndPSPs(filepath, duration, thresh, msPrePeak, tailScaler);
end
