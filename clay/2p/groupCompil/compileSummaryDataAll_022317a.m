...
sessSummStruc.avgTif = mean(tifStack,3);

%% user input soma/dend ('s' or 'd') based upon avgTif
sessSummStruc.somaDend = input('Somata (s) or Dend (d)?', 's');

%% get behav program name
binFilename = findLatestFilename('.bin');
behavBasename = binFilename(1:strfind(binFilename, '.bin')-1);
[sessSummStruc.programName, sessSummStruc.rewDir] = readArduinoProgramNameRewDir([behavBasename '.txt']);
