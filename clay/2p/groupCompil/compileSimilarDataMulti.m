function [batchDendStruc2hz, batchDendStruc4hz, batchDendStruc8hz] = compileSimilarDataMulti(mouseFolderList, masterFieldNamesCaAvg, dataFileArray, paramValCell)
...
% NOW with multiple parameter selection criteria

% find sessions that have certain parameter values
for numParam = 1:size(paramValCell)
    
    paramCol = find(strcmp(paramValCell{numParam, 1}, dataFileArray(1,:))); % 8
    
    
    % find sepChoice data in dataFileArray
    rowIndCell{numParam} = find(strcmp(paramValCell{numParam, 2}, dataFileArray(:, paramCol))); % 8
    
    
end

% find intersection of all of these sessions
for numParam = 1:(size(paramValCell,1)-1)
    
rowIndArr = intersect(rowIndCell{numParam}, rowIndCell{numParam+1});
