...
if length(varargin) == 1 % if one input, then select a single .bin to analyze
...
for fileNum = 1:length(fileDir)
    if strmatch(fileDir(fileNum).name, filename)
...
[eventStruc] = detect2pEventsSingleName(filename,fps);  % detect2pEvents(nChannels, sf);
eventHistBehavStruc.eventStruc = eventStruc;  % just save this into output struc

if isempty(strfind(eventType), 'whiskTrack')
...
for field = 1:length(fieldNames)
    ...
    if ~isempty(eventStruc.(fieldNames{field})) && (length(histEventTimes) > 1)
        ...
        if (~isempty(strfind(fieldNames{field}, 'StimInd')) || ~isempty(strfind(fieldNames{field}, 'Time')))
            ...
            eventTimes = eventTimes((eventTimes > sf*2+1) & (eventTimes < (histEventTimes(end)-sf*6+1)));
