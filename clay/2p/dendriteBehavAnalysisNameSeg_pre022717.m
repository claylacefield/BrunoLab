function [dendriteBehavStruc] = dendriteBehavAnalysisNameSeg(filename, dayPath, seg, toProcess)
...
if toProcess == 1
    ...
    if isempty(strfind(programName, 'stage'))
        disp(['Detecting behavioral events for ' filename]);
            ...
            if (~isempty(strfind(fieldNames{field}, 'StimInd')) || ~isempty(strfind(fieldNames{field}, 'Time')))
                ...
                try
                    ...
                    if ~isempty(C)
                        ...
                        [zeroFrame, eventCa , eventCaAvg, segEventCa, eventRoiHist, eventRoiHist2] = eventTrigCaRoiPk(filename, eventStruc, fps, segStruc, frameAvgDf, field);
