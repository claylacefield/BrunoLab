...
if exist([basename 'b.tif'], 'file')
    tifName = [basename 'b.tif'];
     % for old motion corrected
else
    tifName = [basename '.tif'];
end

[stackInfo] = correctMcTifTagNew(tifName);
...
for i=ch:numImCh:maxFr
    frame = imread(tifName, 'tif', i); % open the TIF frame
    frameNum = frameNum + 1;
