function [dataFileArray] = moveCorrectedSima(dataFileArray, mouseFolderList)
        ...
        for daySessionNum = 3:length(dayFolderDir)
            ...
            if dayFolderDir(daySessionNum).isdir % should only get tif folders
                ...
                if isempty(find(strcmp([tifFolderName '.tif'], dataFileArray(:,1)))) && dayFolderDir(daySessionNum).isdir
                    ...
                    if find(cellfun(@length, strfind({tifFolderDir.name}, [tifFolderName '.sima'])))
                        ...
                        try
                            ...
                            binInd = find(cellfun(@length, strfind({dayFolderDir.name}, '.bin')));  % get dir indices of .txt files
                            binDatenum = [dayFolderDir(binInd).datenum]; % get datenums of .txt files
                            [minDiff, minDiffInd] = min(abs(binDatenum - tifDatenum)); % find the .txt file acquired closest to the .tif file
                            binFileName = dayFolderDir(binInd(minDiffInd)).name;
                            basenameBin = binFileName(1:(strfind(binFileName, '.bin')-1));
                            
                            mouseName = basenameBin(13:(length(basenameBin)-1));
                            ...
                            txtFileName = [basenameBin '.txt'];
                            pngFileName = [basenameBin '.png'];
                            ...
                            disp('WARNING: cannot find .bin file so moving on');
                        end
