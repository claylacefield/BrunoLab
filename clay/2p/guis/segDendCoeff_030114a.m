function varargout = segDendCoeff_030114a(varargin)
% SEGDENDCOEFF_030114A MATLAB code for segDendCoeff_030114a.fig
%      SEGDENDCOEFF_030114A, by itself, creates a new SEGDENDCOEFF_030114A or raises the existing
%      singleton*.
%
%      H = SEGDENDCOEFF_030114A returns the handle to a new SEGDENDCOEFF_030114A or the handle to
%      the existing singleton*.
%
%      SEGDENDCOEFF_030114A('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEGDENDCOEFF_030114A.M with the given input arguments.
%
%      SEGDENDCOEFF_030114A('Property','Value',...) creates a new SEGDENDCOEFF_030114A or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before segDendCoeff_030114a_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to segDendCoeff_030114a_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help segDendCoeff_030114a

% Last Modified by GUIDE v2.5 01-Mar-2014 20:47:43

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @segDendCoeff_030114a_OpeningFcn, ...
    'gui_OutputFcn',  @segDendCoeff_030114a_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before segDendCoeff_030114a is made visible.
function segDendCoeff_030114a_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to segDendCoeff_030114a (see VARARGIN)

% Choose default command line output for segDendCoeff_030114a
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes segDendCoeff_030114a wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = segDendCoeff_030114a_OutputFcn(hObject, eventdata, handles)
...
% % (moved this to segNumSlider callback)
% A = handles.A;
% C = handles.C;
% d1 = handles.d1;
% d2 = handles.d2;
...
maxCoeffSegStruc = importdata('maxCoeffSegStruc_rew_022414a.mat');
handles.maxCoeffSegStruc = maxCoeffSegStruc;

set(handles.segNumSlider, 'Max', length(maxCoeffSegStruc), 'Min', 1);
...
set(handles.RsqrValTxt, 'String', num2str(Rsqr));

goodSegNum = maxCoeffSegStruc(numDend).seg;
set(handles.goodSegNumTxt, 'String', num2str(goodSegNum));
