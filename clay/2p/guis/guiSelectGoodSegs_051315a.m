function varargout = guiSelectGoodSegs_051315a(varargin)
% GUISELECTGOODSEGS_051315A MATLAB code for guiSelectGoodSegs_051315a.fig
%      GUISELECTGOODSEGS_051315A, by itself, creates a new GUISELECTGOODSEGS_051315A or raises the existing
%      singleton*.
%
%      H = GUISELECTGOODSEGS_051315A returns the handle to a new GUISELECTGOODSEGS_051315A or the handle to
%      the existing singleton*.
%
%      GUISELECTGOODSEGS_051315A('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUISELECTGOODSEGS_051315A.M with the given input arguments.
%
%      GUISELECTGOODSEGS_051315A('Property','Value',...) creates a new GUISELECTGOODSEGS_051315A or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before guiSelectGoodSegs_051315a_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to guiSelectGoodSegs_051315a_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help guiSelectGoodSegs_051315a

% Last Modified by GUIDE v2.5 14-May-2015 02:35:19

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @guiSelectGoodSegs_051315a_OpeningFcn, ...
                   'gui_OutputFcn',  @guiSelectGoodSegs_051315a_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before guiSelectGoodSegs_051315a is made visible.
function guiSelectGoodSegs_051315a_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to guiSelectGoodSegs_051315a (see VARARGIN)

% Choose default command line output for guiSelectGoodSegs_051315a
...
% UIWAIT makes guiSelectGoodSegs_051315a wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = guiSelectGoodSegs_051315a_OutputFcn(hObject, eventdata, handles) 
...
%     end
    ...
    axes(handles.axes4);
    bar(x, roiHist1(:,segNum)/numTrials1, width1, 'FaceColor', 'b'); 
    xlim([-8.5 24.5]);
    ylim([0 max(roiHist1(:,segNum))/numTrials1]);
    hold on;
