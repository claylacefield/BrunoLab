function varargout = guiSelectGoodSegs_081217a(varargin)
% GUISELECTGOODSEGS_081217A MATLAB code for guiSelectGoodSegs_081217a.fig
%      GUISELECTGOODSEGS_081217A, by itself, creates a new GUISELECTGOODSEGS_081217A or raises the existing
%      singleton*.
%
%      H = GUISELECTGOODSEGS_081217A returns the handle to a new GUISELECTGOODSEGS_081217A or the handle to
%      the existing singleton*.
%
%      GUISELECTGOODSEGS_081217A('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUISELECTGOODSEGS_081217A.M with the given input arguments.
%
%      GUISELECTGOODSEGS_081217A('Property','Value',...) creates a new GUISELECTGOODSEGS_081217A or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before guiSelectGoodSegs_081217a_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to guiSelectGoodSegs_081217a_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help guiSelectGoodSegs_081217a

% Last Modified by GUIDE v2.5 12-Aug-2017 23:10:59

% Revisions:
% 061917: fixing whisker graph scaling (height and real time)
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @guiSelectGoodSegs_081217a_OpeningFcn, ...
    'gui_OutputFcn',  @guiSelectGoodSegs_081217a_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before guiSelectGoodSegs_081217a is made visible.
function guiSelectGoodSegs_081217a_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to guiSelectGoodSegs_081217a (see VARARGIN)

% Choose default command line output for guiSelectGoodSegs_081217a
...
handles.whiskCheckboxState = 0;
handles.whiskVarCheckboxState = 0;
handles.normHeat = 0;
handles.unitSomaDend = 'dend';
handles.sessionParams.unitSomaDend = {};
...
% UIWAIT makes guiSelectGoodSegs_081217a wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = guiSelectGoodSegs_081217a_OutputFcn(hObject, eventdata, handles)
...
handles.caDataStruc = caDataStruc; % this is same as segStruc
...
set(handles.somaDendListbox,'String',{'dend', 'soma', 'axon', 'dont know'});
...
whiskAngle = whiskDataStruc.meanAngle(1:length(whiskDataStruc.frTimes));
handles.whiskAngle = -(whiskAngle-mean(whiskAngle))/std(whiskAngle);
...
try
    ...
    disp('Cant extract roiHist data');
end
...
try
    ...
    disp('Cant plot roiHist (from listbox1 callback)');
end
...
%        contents{get(hObject,'Value')} returns selected item from listbox1
    ...
    disp('Select another field to plot');
end
...
if trialNumCheckboxState == 1
    ...
    trialSliderVal = round(get(hObject, 'Value'));
...
whiskVarCheckboxState = handles.whiskVarCheckboxState;
field1Base = handles.field1Base;
field2Base = handles.field2Base;
dendriteBehavStruc = handles.dendriteBehavStruc;
whiskSig = handles.whiskSig;
frTimes = handles.whiskFrTimes;
whiskAngle = handles.whiskAngle; % already Zscored

if whiskVarCheckboxState == 1
    whiskAngle = movingvar(whiskAngle', 30);
end
...
trueFps = 1000/mean(diff(frameTrig));

preEvMs = round(1000*8/trueFps);
postEvMs = round(1000*24/trueFps);
...
for i = 1:length(field1ZeroTimes)
    if field1FrInds(i)-8 > 0
        ...
        field1StartWhiskFrames(i) = find(frTimes>=(frameTrig(1)), 1);
        field1EndWhiskFrames(i) = find(frTimes<=(frameTrig(33)), 1, 'last');
    end
end

for i = 1:length(field2ZeroTimes)
    try
        field2StartWhiskFrames(i) = find(frTimes>(frameTrig(field2FrInds(i)-8)), 1);
        field2EndWhiskFrames(i) = find(frTimes<(frameTrig(field2FrInds(i)+24)), 1, 'last');
    catch
    end
end
...
%         eventWhiskAngleTrial = eventWhiskAngleTrial-min(eventWhiskAngleTrial);
%         eventWhiskAngleTrial = eventWhiskAngleTrial/max(eventWhiskAngleTrial);
        eventWhiskAngle(:,i) = interp1(frTimes(eventStart(i):eventEnd(i)), eventWhiskAngleTrial, round(linspace(frTimes(eventStart(i)),frTimes(eventEnd(i)), preEvMs+postEvMs+1)));
    end
    ...
    axes(handles.axes5);
    %ylim([-100 100]);
    plot(handles.axes5, meanWhiskAngle); % 1:8000, meanWhiskAngle);
...
%         postEvMs = round(1000*24/trueFps);
        ...
        trialSliderVal = handles.trialSliderVal; % read trial slider value
        preTrialMs = field1ZeroTimes(trialSliderVal)-preEvMs;
        postTrialMs = field1ZeroTimes(trialSliderVal)+postEvMs;
        whiskSigTrial = whiskSig(preTrialMs:postTrialMs);   % whisker contact signal (IR)
        whiskSigTrialEpoch = whiskSig((field1ZeroTimes(trialSliderVal)+500:field1ZeroTimes(trialSliderVal)+3000)); % establish actual trial epoch for scaling
        %whiskSigTrial = whiskAngle(eventStart(trialSliderVal):eventEnd(trialSliderVal));
        whiskSigTrial = (whiskSigTrial-mean(whiskSigTrialEpoch))/2; %max(whiskSigTrialEpoch);
        ...
        whiskAngleTrial = eventWhiskAngle(:,trialSliderVal);
        whiskAngleTrial = whiskAngleTrial/max(abs(whiskAngleTrial));
        plot(handles.axes5, preTrialMs:postTrialMs, whiskAngleTrial+1);
...
sessionParams = handles.sessionParams;
...
save([filename(1:14) '_sessionParams_' goodSegTag '_' segDate], 'sessionParams');
...
try
    if handles.normHeat == 1
        ca2 = sortUnitsByTime(segStruc, goodSeg, 0,0,1,0); %sortEvent, plotEvent, toNorm)
    else
        ca2 = sortUnitsByTime(segStruc, goodSeg, 0,0,0,0);
    end
    disp('Plotting heatmap');
    axes(handles.goodSegHeatmap);
    imagesc(ca2);
...
handles.sessionParams.unitSomaDend{segNum} = handles.unitSomaDend; % somaDendCheckbox;
...
plot(-1000:1000,100*(segCaXc-mean(segCaXc))/mean(segCaXc), 'g'); 
xlim([-1000 1000]);
ylabel('xcorr');
hold off;
...
% --- Executes on button press in dendCheckbox.
function dendCheckbox_Callback(hObject, eventdata, handles)
% hObject    handle to dendCheckbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)


    handles.somaDendCheckbox = get(hObject,'Value');

 %   handles.sessionParams.unitSomaSend{segNum} = 'soma';


guidata(hObject, handles);

function layerTextbox_Callback(hObject, eventdata, handles)
% hObject    handle to layerTextbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

handles.sessionParams.layer = get(hObject,'String'); % returns contents of layerTextbox as text
% str2double(get(hObject,'String')) returns contents of layerTextbox as a double

guidata(hObject, handles);


% --- Executes during object creation, after setting all properties.
function layerTextbox_CreateFcn(hObject, eventdata, handles)
% hObject    handle to layerTextbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in somaDendListbox.
function somaDendListbox_Callback(hObject, eventdata, handles)
% hObject    handle to somaDendListbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

somaDendList = cellstr(get(hObject,'String')); % returns somaDendListbox contents as cell array
listInd = get(hObject,'Value'); %  returns selected item from somaDendListbox
handles.unitSomaDend = somaDendList{listInd};

guidata(hObject, handles);


% --- Executes during object creation, after setting all properties.
function somaDendListbox_CreateFcn(hObject, eventdata, handles)
% hObject    handle to somaDendListbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in whiskVarCheckbox.
function whiskVarCheckbox_Callback(hObject, eventdata, handles)
% hObject    handle to whiskVarCheckbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of whiskVarCheckbox


handles.whiskVarCheckboxState = get(hObject,'Value');
...
% --- Executes on button press in normHeatBox.
function normHeatBox_Callback(hObject, eventdata, handles)
% hObject    handle to normHeatBox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of normHeatBox


handles.normHeat = get(hObject, 'Value');
