function varargout = guiSelectGoodSegs_012216a(varargin)
% GUISELECTGOODSEGS_012216A MATLAB code for guiSelectGoodSegs_012216a.fig
%      GUISELECTGOODSEGS_012216A, by itself, creates a new GUISELECTGOODSEGS_012216A or raises the existing
%      singleton*.
%
%      H = GUISELECTGOODSEGS_012216A returns the handle to a new GUISELECTGOODSEGS_012216A or the handle to
%      the existing singleton*.
%
%      GUISELECTGOODSEGS_012216A('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUISELECTGOODSEGS_012216A.M with the given input arguments.
%
%      GUISELECTGOODSEGS_012216A('Property','Value',...) creates a new GUISELECTGOODSEGS_012216A or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before guiSelectGoodSegs_012216a_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to guiSelectGoodSegs_012216a_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help guiSelectGoodSegs_012216a

% Last Modified by GUIDE v2.5 24-Jan-2016 02:49:11

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @guiSelectGoodSegs_012216a_OpeningFcn, ...
    'gui_OutputFcn',  @guiSelectGoodSegs_012216a_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before guiSelectGoodSegs_012216a is made visible.
function guiSelectGoodSegs_012216a_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to guiSelectGoodSegs_012216a (see VARARGIN)

% Choose default command line output for guiSelectGoodSegs_012216a
...
% UIWAIT makes guiSelectGoodSegs_012216a wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = guiSelectGoodSegs_012216a_OutputFcn(hObject, eventdata, handles)
...
field1StartWhiskFrames(i) = find(frTimes>(field1ZeroTimes(i)-2000), 1);
field1EndWhiskFrames(i) = find(frTimes<(field1ZeroTimes(i)+6000), 1, 'last');
end

for i = 1:length(field2ZeroTimes)
field2StartWhiskFrames(i) = find(frTimes>(field2ZeroTimes(i)-2000), 1);
field2EndWhiskFrames(i) = find(frTimes<(field2ZeroTimes(i)+6000), 1, 'last');
end
