function varargout = segDendCaPlotFieldsAllSEM_052814a(varargin)
% SEGDENDCAPLOTFIELDSALLSEM_052814A MATLAB code for segDendCaPlotFieldsAllSEM_052814a.fig
%      SEGDENDCAPLOTFIELDSALLSEM_052814A, by itself, creates a new SEGDENDCAPLOTFIELDSALLSEM_052814A or raises the existing
%      singleton*.
%
%      H = SEGDENDCAPLOTFIELDSALLSEM_052814A returns the handle to a new SEGDENDCAPLOTFIELDSALLSEM_052814A or the handle to
%      the existing singleton*.
%
%      SEGDENDCAPLOTFIELDSALLSEM_052814A('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEGDENDCAPLOTFIELDSALLSEM_052814A.M with the given input arguments.
%
%      SEGDENDCAPLOTFIELDSALLSEM_052814A('Property','Value',...) creates a new SEGDENDCAPLOTFIELDSALLSEM_052814A or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before segDendCaPlotFieldsAllSEM_052814a_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to segDendCaPlotFieldsAllSEM_052814a_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help segDendCaPlotFieldsAllSEM_052814a

% Last Modified by GUIDE v2.5 28-May-2014 14:06:05

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @segDendCaPlotFieldsAllSEM_052814a_OpeningFcn, ...
                   'gui_OutputFcn',  @segDendCaPlotFieldsAllSEM_052814a_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before segDendCaPlotFieldsAllSEM_052814a is made visible.
function segDendCaPlotFieldsAllSEM_052814a_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to segDendCaPlotFieldsAllSEM_052814a (see VARARGIN)

% Choose default command line output for segDendCaPlotFieldsAllSEM_052814a
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes segDendCaPlotFieldsAllSEM_052814a wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = segDendCaPlotFieldsAllSEM_052814a_OutputFcn(hObject, eventdata, handles) 
...
load(filename);  % load in the segStruc
caDataStruc = segStruc;

% handles.typeStruc = fieldNames{1};
handles.caDataStruc = caDataStruc;  % save to handles struc

% need to find fields in calcium data structure
fieldNames = fieldnames(caDataStruc);   % get all fieldnames
...
fieldNames = fieldNames(caFieldInd);    % so now only getting 'Ca' fields
