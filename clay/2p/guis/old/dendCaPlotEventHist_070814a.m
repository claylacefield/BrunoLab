function varargout = dendCaPlotEventHist_070814a(varargin)
% DENDCAPLOTEVENTHIST_070814A MATLAB code for dendCaPlotEventHist_070814a.fig
%      DENDCAPLOTEVENTHIST_070814A, by itself, creates a new DENDCAPLOTEVENTHIST_070814A or raises the existing
%      singleton*.
%
%      H = DENDCAPLOTEVENTHIST_070814A returns the handle to a new DENDCAPLOTEVENTHIST_070814A or the handle to
%      the existing singleton*.
%
%      DENDCAPLOTEVENTHIST_070814A('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DENDCAPLOTEVENTHIST_070814A.M with the given input arguments.
%
%      DENDCAPLOTEVENTHIST_070814A('Property','Value',...) creates a new DENDCAPLOTEVENTHIST_070814A or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before dendCaPlotEventHist_070814a_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to dendCaPlotEventHist_070814a_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help dendCaPlotEventHist_070814a

% Last Modified by GUIDE v2.5 08-Jul-2014 22:01:03

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @dendCaPlotEventHist_070814a_OpeningFcn, ...
                   'gui_OutputFcn',  @dendCaPlotEventHist_070814a_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before dendCaPlotEventHist_070814a is made visible.
function dendCaPlotEventHist_070814a_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to dendCaPlotEventHist_070814a (see VARARGIN)

% Choose default command line output for dendCaPlotEventHist_070814a
...
% UIWAIT makes dendCaPlotEventHist_070814a wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = dendCaPlotEventHist_070814a_OutputFcn(hObject, eventdata, handles) 
...
for numFile = 1:length(currDir)
   if ~isempty(strfind(currDir(numFile).name, 'ventHist')) && firstFile == 0
       inStruc = load(currDir(numFile).name);
...
%     try
        ...
        if size(eventHist1, 2)>1
            eventHist1 = mean(eventHist1, 2);
            
        end
        
        axes(handles.axes5);
        bar(eventHist2/max(eventHist2), 'r'); xlim([0 8000]); hold on;
        bar(eventHist1/max(eventHist1), 'b'); hold off;
%     catch
%     end
