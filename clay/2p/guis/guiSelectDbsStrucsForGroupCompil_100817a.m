function varargout = guiSelectDbsStrucsForGroupCompil_100817a(varargin)
% GUISELECTDBSSTRUCSFORGROUPCOMPIL_100817A MATLAB code for guiSelectDbsStrucsForGroupCompil_100817a.fig
%      GUISELECTDBSSTRUCSFORGROUPCOMPIL_100817A, by itself, creates a new GUISELECTDBSSTRUCSFORGROUPCOMPIL_100817A or raises the existing
%      singleton*.
%
%      H = GUISELECTDBSSTRUCSFORGROUPCOMPIL_100817A returns the handle to a new GUISELECTDBSSTRUCSFORGROUPCOMPIL_100817A or the handle to
%      the existing singleton*.
%
%      GUISELECTDBSSTRUCSFORGROUPCOMPIL_100817A('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUISELECTDBSSTRUCSFORGROUPCOMPIL_100817A.M with the given input arguments.
%
%      GUISELECTDBSSTRUCSFORGROUPCOMPIL_100817A('Property','Value',...) creates a new GUISELECTDBSSTRUCSFORGROUPCOMPIL_100817A or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before guiSelectDbsStrucsForGroupCompil_100817a_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to guiSelectDbsStrucsForGroupCompil_100817a_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help guiSelectDbsStrucsForGroupCompil_100817a

% Last Modified by GUIDE v2.5 08-Oct-2017 23:29:40

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @guiSelectDbsStrucsForGroupCompil_100817a_OpeningFcn, ...
    'gui_OutputFcn',  @guiSelectDbsStrucsForGroupCompil_100817a_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
%% --- Executes just before guiSelectDbsStrucsForGroupCompil_100817a is made visible.
function guiSelectDbsStrucsForGroupCompil_100817a_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to guiSelectDbsStrucsForGroupCompil_100817a (see VARARGIN)

% Choose default command line output for guiSelectDbsStrucsForGroupCompil_100817a
handles.output = hObject;

% Update handles structure
%guidata(hObject, handles);

% UIWAIT makes guiSelectDbsStrucsForGroupCompil_100817a wait for user response (see UIRESUME)
...
handles.groupUnitRewCa1 = [];
handles.groupUnitRewCa2 = [];
...
function varargout = guiSelectDbsStrucsForGroupCompil_100817a_OutputFcn(hObject, eventdata, handles)
...
% try
    ...
    handles.segStruc1 = segStruc;
    handles.goodSeg1 = goodSeg;
    handles.unitRewCa1 = segStruc.correctRewStimIndCaAvg(:,goodSeg);
    plotUnitCa(handles, 1);
    
    axes(handles.unitAvgCaAxes);
    plot(mean(handles.unitRewCa1,2), 'b');
    hold on;
    try
        plot(mean(handles.unitRewCa2,2), 'g');
    catch
    end
    hold off;
    
% catch
%     disp('No group1 goodSeg selected');
%     %handles.goodSeg2 = [];
% end
...
try
    load(findLatestFilename('_goodSeg_'));
    handles.segStruc2 = segStruc;
    handles.goodSeg2 = goodSeg;
    handles.unitRewCa2 = segStruc.correctRewStimIndCaAvg(:,goodSeg);
    plotUnitCa(handles, 2);
    ...
    disp('No group2 goodSeg selected');
    %handles.goodSeg2 = [];
end
...
% group1segStruc = handles.group1segStruc;
% [group1segStruc] = groupStrucCompile(group1segStruc, fieldNames1, struc1);
% handles.group1segStruc = group1segStruc;

handles.groupUnitRewCa1 = [handles.groupUnitRewCa1 handles.unitRewCa1];
...
handles.groupUnitRewCa2 = [handles.groupUnitRewCa2 handles.unitRewCa2];
...
groupUnitRewCa1 = handles.groupUnitRewCa1;
groupUnitRewCa2 = handles.groupUnitRewCa2;
...
save(filename, 'group1Struc', 'group2Struc', 'group1fileNames', 'group2fileNames', 'groupUnitRewCa1', 'groupUnitRewCa2');
...
plotCaData(handles);
plotUnitCa(handles, 3);

axes(handles.unitAvgCaAxes);
if handles.toPlotGroup~=0
    avCa1 = handles.groupUnitRewCa1;
    try
        avCa2 = handles.groupUnitRewCa2;
    catch
    end
else
    avCa1 = handles.unitRewCa1;
    try
        avCa2 = handles.unitRewCa2;
    catch
    end
end
plot(mean(avCa1,2), 'b');
...
function plotUnitCa(handles, groupNum)


% Plan:
% When file selected for either group, load segStruc/goodSeg, and
% plotUnitsByColor, or if plotGroup clicked, plot group (both)

if groupNum ==1
    axes(handles.unitColorPlot1axes);
    avCa = handles.unitRewCa1;
    plotUnitsByTime4(avCa);
elseif groupNum ==2
    axes(handles.unitColorPlot2axes);
    avCa = handles.unitRewCa2;
    plotUnitsByTime4(avCa);
elseif groupNum ==3     % or plot groups
    axes(handles.unitColorPlot1axes);
    avCa = handles.groupUnitRewCa1;
    plotUnitsByTime4(avCa);
    try
    axes(handles.unitColorPlot2axes);
    avCa = handles.groupUnitRewCa2;
    plotUnitsByTime4(avCa);
    catch
        disp('No group2');
    end
