function [correctRespStruc] = correctResp2pSession()

% [correctRespStruc] = correctResp2pSession();
% 050617: This is a modification that parses .txt behavioral data from
% arduino and also outputs arduino time for the trials (and rew?)
...
tifInd = find(cellfun(@length, strfind({currDir.name}, '.tif')));
tifDatenum = [currDir(tifInd).datenum];
