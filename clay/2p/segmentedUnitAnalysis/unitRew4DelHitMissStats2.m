function [unitRew4DelayStats] = unitRew4DelHitMissStats2(unitRewDelayStruc)
...
%% make binary array of calcium fields wderith all data types
goodCellField = [];
goodCells = ones(1,length(unitRewDelayStruc.(fieldNames{5}))); % numCaField = 0;
for field = 1:length(fieldNames)
    
    if ~isempty(strfind(fieldNames{field}, 'TimeCa')) && isempty(strfind(fieldNames{field}, 'CaAvg'))
...
rewEpRateCell = unitRewDelayStruc.rewEpRateCell(goodCellInds);
rewEpRateMat = []; 
rewStimCa = {unitRewDelayStruc.rewStimCa{goodCellInds}};
rewStimCaArr = [];
...
unitRew4DelayStats.unitRandRewPval = [unitRewDelayStruc.unitRandRewPvalCell{goodCellInds}];
unitRew4DelayStats.rewStimCa = rewStimCaArr;

unitRew4DelayStats.segDiffInd = segDiffInd;
unitRew4DelayStats.segDelSlope = segDelSlope;
%unitRew4DelayStats.segHitMissDiff = segHitMissDiff;
