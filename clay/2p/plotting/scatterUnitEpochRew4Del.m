function scatterUnitEpochRew4Del(rewEpRateMat, randRewUnits, rewTrackUnits)

pvalThresh = 0.02;
sigRew4UnitInd = randRewUnits; %find(unitRandRewPval <= pvalThresh);
sigUnitInd = rewTrackUnits; %find(segDiffInd == 0);

%rateRatio = mouseUnitEpochSortStruc.rewEpRate(:,2)./mouseUnitEpochSortStruc.rewEpRate(:,1);

rewEpRate1 = squeeze(rewEpRateMat(:,1,1));
...
rewEpRate2 = max([squeeze(rewEpRateMat(:,2,1)) squeeze(rewEpRateMat(:,3,1))],[],2);
