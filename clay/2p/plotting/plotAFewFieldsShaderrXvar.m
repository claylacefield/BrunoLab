function [fullSessXcorr, fullSessSEM] = plotAFewFieldsShaderrXvar(inStruc, fields, baselineFrames, x) % , useMice)
...
%
% Now, this is for whiskCa Xcorr, not trial-avg ca
%
...
for field = 1:length(fields)
    ...
    fieldCa = fieldCa(lims(1):lims(2), :);
    fieldCa = fieldCa(1:10:end,:);
...
line([0 0], [-0.3, 0.3], 'Color', [0.65 0.65 0.65]);
...
xlim([-200 200]);
ylim([-0.3 0.3]); %([minY-0.08, maxY+0.08]);
