function [frTimes, frameRate, frTopAv, errorCell] = extractStimFrames(filename, dendriteBehavStruc, saveMem)
...
try  % TRY/CATCH for problems, such as if stim isn't visible in frame, or if there is no stim (e.g. stage1)
    ...
    disp('Reading number of frames'); tic;
%     width = vob.width;
%     height = vob.height;
    frameRate = vob.FrameRate; 
    ...
    if saveMem == 0
        disp(['Reading entire video for ' filename]); tStart = tic;
        ...
        disp(['Computing average pixel lum for top ' num2str(topRange) ' rows of video']); tStart = tic;
        frTopAv = [];
        ...
        disp(['Reading ' filename ' video frame by frame while ']);
        tic;
        disp(['Computing average pixel lum for top ' num2str(topRange) ' rows of video']); tStart = tic;
        frTopAv = [];
        for frNum = 1:numWhVidFrames
            ...
            lines = frame(1:topRange,1:3*round(size(frame,2)/4));
            lines = lines(:);  % just linearize
            lines(lines>3*std(lines)+mean(lines)) = 0; % if pix really bright, then zero
            frTopAv(frNum) = mean(lines);  % and take the average value
        end
        toc;
        clear vob; toc(tStart);
    end
