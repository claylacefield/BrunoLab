...
wdsInd = find(cellfun(@length, strfind(sessionDirNames, 'whiskDataStruc')));
wdsDatenums = [sessionDir(wdsInd).datenum];
[lsaMax, lastInd] = max(wdsDatenums);
latestWdsInd = wdsInd(lastInd);
whiskDataStrucName = sessionDir(latestWdsInd).name;
