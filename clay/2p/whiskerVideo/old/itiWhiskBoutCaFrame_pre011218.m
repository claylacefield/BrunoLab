...
lockoutSec = 0.5; %1; %2 (old paper?);
sdThresh = 0.75; %1 (old paper);%2; %1.5;
...
%wPks2 = LocalMinima(-ma4, lockout, -sdThresh*std(ma4));
...
% t = 1:length(ma);
% figure; plot(ma3); 
% hold on; plot(t(wPks), ma3(wPks), 'r.');
...
% 011218
cleanStimInd = cleanStimInd(2:end);

cleanItiInd = cleanStimInd-1;
...
[whiskBoutCaStruc] = calcItiWhiskCa(cleanStimTrigTime, timeMs, wPkTimes, dendriteBehavStruc, v, ma, params);
...
% try
    [whiskBoutCaStruc] = calcItiWhiskCa(stimTrigTime(stimAfterRand), timeMs, wPkTimes, dendriteBehavStruc, v, ma, params);
