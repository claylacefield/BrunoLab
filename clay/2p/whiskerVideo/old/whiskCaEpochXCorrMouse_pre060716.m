            ...
            if dayDir(dayDirInd).isdir
                ...
                if ~isempty(find(cellfun(@length, strfind(sessionFilenames, 'whiskDataStruc'))))
                    ...
                    disp(['Processing whiskCaEpochXCorr for ' sessionName]); tic;
                        ...
                        whCaXCorrGroupStruc.allITIs(:,numSessions) = whiskCaXCorrStruc.allITIs;
                        whCaXCorrGroupStruc.randRewITIs(:,numSessions) = whiskCaXCorrStruc.randRewITIs;
                        whCaXCorrGroupStruc.stimTrials(:,numSessions) = whiskCaXCorrStruc.stimTrials;
                        whCaXCorrGroupStruc.catchTrials(:,numSessions) = whiskCaXCorrStruc.catchTrials;
                        whCaXCorrGroupStruc.fullSession(:,numSessions) = whiskCaXCorrStruc.fullSession;
                    catch
