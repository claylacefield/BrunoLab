...
pkV = centers(vStartPks(1:3));  % find common voltage val
% NOTE: only taking the lower vale, i.e. not the TTL signal, for shift in
% baseline with stim onset
pkCounts = counts(vStartPks(1:3));  % and how common
...
itiInd = find(abs(rmStart-itiVal) < 0.03);
...
% find iti end based upon big gaps between iti indices
itiEndInd = itiInd(find(diff(itiInd)>1000));

plot(t(itiEndInd), rmStart(itiEndInd), 'y.');

% and likewise for iti beginning
