...
rewStimXcorr = allStimXcorr((dendriteBehavStruc.eventStruc.rewStimStimInd <= size(allStimXcorr,1)), :);
unrewStimXcorr = allStimXcorr((dendriteBehavStruc.eventStruc.unrewStimStimInd <= size(allStimXcorr,1)), :);

correctRewXcorr = allStimXcorr((dendriteBehavStruc.eventStruc.correctRewStimInd <= size(allStimXcorr,1)), :); 
if isempty(correctRewXcorr)
    correctRewXcorr =  NaN(1,6001);
end

incorrectRewXcorr = allStimXcorr((dendriteBehavStruc.eventStruc.incorrectRewStimInd <= size(allStimXcorr,1)), :);
if isempty(incorrectRewXcorr)
   incorrectRewXcorr =   NaN(1,6001);
end


correctUnrewXcorr = allStimXcorr((dendriteBehavStruc.eventStruc.correctUnrewStimInd <= size(allStimXcorr,1)), :); 
if isempty(correctUnrewXcorr)
correctUnrewXcorr =  NaN(1,6001);
end

incorrectUnrewXcorr = allStimXcorr((dendriteBehavStruc.eventStruc.incorrectUnrewStimInd <= size(allStimXcorr,1)), :);
