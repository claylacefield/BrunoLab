function [correctRespStruc] = correctResp2pSingleName(sessionBasename);
...
initRew = 0;
txtInfo = dir(filename);
txtDatenum = txtInfo.datenum;
    ...
    if strfind(event1, 'bottom')
        ...
        if strfind(event2, 'REWARD')
            ...
            elseif strfind(event3, 'reward solenoid skip');
                ...
                corrRewInd = corrRewInd + 1;
                correctRewStimInd(corrRewInd) = numStim; % stim time for correct lever presses
                correctRewLatency(corrRewInd) = ev2time - ev1time;
                
                rewInd = rewInd + 1;
                rewStimInd(rewInd) = numStim; % 072713: stim indices of normal rewards
                rewArrInd = rewArrInd + 1;
                rewArr(rewArrInd) = 1;  % "1" is normal reward in rewArrnpru
                rewArrArdT(rewArrInd) = ev1time;
...
%             incorrectUnrewLatency(incorrUnrewInd) = ev2time - ev1time;
...
try
    correctRespStruc.correctRewLatency = correctRewLatency;
catch
    correctRespStruc.correctRewLatency = [];
end
...
try
    correctRespStruc.incorrectUnrewLatency = incorrectUnrewLatency;
catch
    correctRespStruc.incorrectUnrewLatency = [];
end
...
try
    correctRespStruc.rewArrArdT = rewArrArdT; % for each reward, tells what kind (for skips, switches)
catch
    correctRespStruc.rewArrArdT = [];
end
...
try
    correctRespStruc.punArrArdT = punArrArdT; % same dimen as stimTypeArr and 1 for correct, 0 for incorrect
catch
    correctRespStruc.punArrArdT = [];
end
