function  dendriteBehavAnalysisMouseSeg2simaNew(seg, saveTif, filterDate)
...
%                 end
                ...
                rowInd = 1; % not using dataFileArray right now
...
%                         end
                        ...
                        % use stage1 if there is some problem
                        if isempty(eventStruc.rewStimStimInd)
                            disp('Cant find normal events so using stage1 event detection');
                            eventStruc = detect2pEventsNameStage1New(sessBasename);
                        end
