...
% also want to look at lifts right before rewards and punishments
...
frameTrig = LocalMinima(galvoSig, 200, -0.2);
...
% use dataFileArray to pick TXT behavioral data file and analyze
...
stimEpochEnd = stimEpochStart + 3*sf;  % and stop just before the end motor artifact
...
whiskContacts = LocalMinima(-filtWhiskSig, 50, -0.1); 
...
% now limit to trials where there are contacts
firstContactTimes = firstContactArrAbsT(firstContactArrAbsT ~= 0);
...
corrFirstContactTimes = corrFirstContactTimes(corrFirstContactTimes ~= 0);
incorrFirstContactTimes = firstContactArrAbsT(incorrRewInd);
incorrFirstContactTimes = incorrFirstContactTimes(incorrFirstContactTimes ~= 0);
