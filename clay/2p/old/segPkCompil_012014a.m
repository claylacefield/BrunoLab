                ...
                for b = 3:length(dayDir);   % for each file in this day (now, folder with tif)
                    if dayDir(b).isdir
                        ...
                        if rowInd
                                    ...
                                    if ~isempty(strfind(fieldNames{field}, 'CaAvg'))
                                        ...
                                        try
                                            ...
                                            for roiNum = 1:length(goodSeg)  % for all good segments
                                                segEventCa = eventCa(:,goodSeg(roiNum));    % dim = 33x1
                                                ...
                                                [caPkVal, caPkInd] = max(segEventCa(7:16)); % find peak
                                                caAvgPkVal(numField, roiNum) = caPkVal-mean(segEventCa(4:6));
                                                caAvgPkInd(numField, roiNum) = caPkInd+6;
                            ...
                            % numSessions)
                            segPkStruc.mouse(mouse).session(numSessions).name = fn;
%                             segPkStruc.mouse(mouse).session(numSessions).fieldNames = fieldNames; % NOTE: this is ALL fieldNames (i.e. even ones not processed)
                            segPkStruc.mouse(mouse).session(numSessions).goodSeg = goodSeg;
                            segPkStruc.mouse(mouse).session(numSessions).caAvgPkVal = caAvgPkVal;
                            segPkStruc.mouse(mouse).session(numSessions).caAvgPkInd = caAvgPkInd;
%                             segPkStruc.mouse(mouse).session(numSessions).fieldInd = fieldInd;   % use this with fieldNames to find which fields processed into caPkVal
                            segPkStruc.mouse(mouse).session(numSessions).fieldNames = fieldNames(fieldInd); 
                            ...
                            clear caPkVal caPkInd fieldInd fieldNames goodSeg;
