function [roiStruc, frameAvgDf] = dendriteProfiles(numFrames, avg, roi)
...
if avg == 1
    s = uiimport;
    imageJresults2 = s.wholeFrameAv;
    clear s;
    
    frameAvg = imageJresults2(:,2);
    frameAvgAvg = mean(frameAvg); % take means over all frames
    frameAvgDf = (frameAvg-frameAvgAvg)/frameAvgAvg; % calculate dF/F for this ROI
