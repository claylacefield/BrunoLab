...
eventStruc.binFilename = dataFileArray{rowInd, 3};
...
% NOTE: changed method a bit 042414 because some signals small
 ...
 dStim = runmean(stim, 20);
 %dStim2 = dStim-runmean(stim, 100);
 %dStim = diff(stim2);
 %dStim = [0 dStim];
 stdDstim = std(dStim);
 
stimTrigThresh =  2*stdDstim; % 0.2;  %

% find beginning of start trigger step pulse (as large pos slope)
stimTrig = LocalMinima(-dStim, 2*sf, -stimTrigThresh);  % had 4*sf timeout before 051914
...
% adding this in 071714 because of R5 animals with really long rew pulses
...
%         rewTime = rewTime((length(rewTime)-length(rewArr)+1):end);
%         disp('truncating rewTime to rewArr length');
%         % NOTE: is the problem more rewards after stopping behavior (in
%         % which case there would be reward signal and no rewArr event)? NO
%         % I think the problem is initial rewards not captured in the txt
%         % file...

% 051914 No, actually its usually a signal at the end not in rewArr


    else
        rewArr = rewArr(1:length(rewTime));
        disp('truncating rewArr to rewTime length');
    end
...
% rewInd4 = rewInd4(rewInd4 <= length(rewTime));
...
eventStruc.incorrUnrewFirstContactTimes2 = incorrUnrewFirstContactTimes2(incorrUnrewFirstContactTimes2 ~= 0);
    ...
    disp('ERROR: problem with firstContactTimes2');
end
...
sdGalvoMax = std(galvoSig(frameTrig));
frameTrig = frameTrig(galvoSig(frameTrig)<(galvoAvMax + 4*sdGalvoMax)); 
