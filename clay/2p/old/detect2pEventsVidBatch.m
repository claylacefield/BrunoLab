function [eventStruc, whiskSig1, x2] = detect2pEventsVidBatch(dataFileArray, rowInd)
...
eventStruc.rewTime1 = rew(rewInd1);    % time of normal rewards
eventStruc.rewTime2 = rew(rewInd2);     % rewTone skip rewards
eventStruc.rewTime3 = rew(rewInd3);     % normal punRew switch rewards
eventStruc.rewTime4 = rew(rewInd4);     % random rewards
...
eventStruc.rewTime7 = rew(rewInd7);
...
whiskSig1 = whiskVideoProcBatch(dataFileArray, rowInd);
timeMs = length(whiskSig2);
whiskFrameTrig = linspace(0, timeMs, length(whiskSig1));

filtWhiskSig1 = whiskSig1;
...
% now doing same but for video rates
timeoutMs = 500;  % how many ms minimum between whisk touches
timeoutFrames = 10;    % figures out how many samples in timeout
stdThresh1 = 0.5*std(filtWhiskSig1);  % stdev threshold for whisker touch detection
    

whiskContacts1 = LocalMinima(filtWhiskSig1, timeoutFrames, -stdThresh1);  %  (-whiskSig1, timeoutSamp, -(stdThresh*stdWhisk));
whiskContactTime1 = whiskFrameTrig(whiskContacts1);
eventStruc.whiskContactTime1 = whiskContactTime1;
...
for k = 1:length(stimTrig)   % so for each stimulus trigger in the corresponding stimulus epoch
    ...
    whiskContactTrial1 = whiskContactTime1(whiskContactTime1 >= stimEpochStart(k) & whiskContactTime1 <= stimEpochEnd(k));
