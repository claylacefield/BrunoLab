...
frameAvg = squeeze(mean(mean(tifStack, 2),1));
frameAvgMean = mean(frameAvg, 1);
frameAvgDf2 = (frameAvg-frameAvgMean)/frameAvgMean;

numFrames = sizeStack;

frameSec = linspace(0,length(frameAvg)/hz,length(frameAvg));  % calculate the axis of frames in terms of secs
...
if avg = 1 | roi = 1; % specify wheter to process frame mean profile and ROI profiles
    [roiStruc, frameAvgDf] = dendriteProfiles(numFrames, avg, roi);
else
    roiStruc = []; frameAvgDf = frameAvgDf2;
end
