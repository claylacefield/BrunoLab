function  dendriteBehavAnalysisBatchSeg2(dataFileArray, seg, saveTif)
...
%                             try  % with CATCH around 467
                            ...
                            if seg == 1
                                ...
                                for d = 3:length(tifDir)
                                    if (~isempty(strfind(tifDir(d).name, 'seg2')) && prevSeg == 0)
...
%                                         roiThresh = max(1*roiSd, 0.1);
                                        roiThresh = 0.04;
                                        roiRm = runmean([0; diff(roiDf)],2);
                                        
                                        roiPkInd = LocalMinima(-roiRm, 4, -roiThresh);   % indices of Ca2+ pks
                                        roiPkAmp{roiNum} = roiRm(roiPkInd);     % amp of detected pks
                                        roiPks(roiPkInd) = 1;
                                        ...
                                        roiPkInd2 = LocalMinima(-roiDf, 4, -roiThresh);   % indices of Ca2+ pks
