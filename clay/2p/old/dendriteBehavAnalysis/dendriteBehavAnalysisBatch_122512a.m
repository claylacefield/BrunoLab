function [batchDendriteBehavStruc] = dendriteBehavAnalysis(dataFileArray)
...
for a = 3:length(parentDir) % for each day of imaging in this animal's dir
    ...
    cd([parentFolder '/' parentDir(a).name]); % go to this day of imaging
    
    dayDir = dir;
    
    for b = 3:length(dayDir);   % for each file in this day
        
        % see if this file is on the dataFileArray list
        % if strfind(dayDir(b).name, dataFileArray{x,1})
        ...
        [eventStruc] = detect2pEvents(nChannels,1000);  % detect2pEvents(nChannels, sf);
        ...
        for field = 1:length(fieldNames)
            ...
            if length(eventStruc.(fieldNames{field}))>0
