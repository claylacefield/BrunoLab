                ...
                for field = 1:length(fieldNames)
                    ...
                    if ~isempty(eventStruc.(fieldNames{field}))
                        ...
                        if isempty(strfind(fieldNames{field}, 'Trig')) && isempty(strfind(fieldNames{field}, 'AbsT')) && isempty(strfind(fieldNames{field}, 'licks'))
                            ...
                            % 052213 changed this to eliminate events that
                            % might cause error at beginning and end 
                            eventTimes = eventTimes(eventTimes > frameTrig(preFrame) & eventTimes < frameTrig((end-postFrame)));
                            
                            % eventEpochTool; % use subfunction to calculate event-trig Ca activ
                            
                            % find the indices of the first frames after
                            % any given event type 
                            for j=1:length(eventTimes)
                                zeroFrame(j) = find(frameTrig >= eventTimes(j), 1, 'first');
                            end
                            
                            % then find indices of a window of frames
                            % around the events
                            ...
                            % so concatenates event Ca averages for this
                            % animal to all previous 
