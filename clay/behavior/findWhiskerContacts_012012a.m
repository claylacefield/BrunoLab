...
while ~feof(fid)
    ...
    stimEpochStart = stimTrig{j} + 10000;   % start looking for whisker contacts after motor artifact should have ended
    stimEpochEnd = stimEpochStart + 40000;  % and stop just before the end motor artifact
    ...
    for k = 1:length(stimTrig{j})   % so for each stimulus trigger in the corresponding stimulus epoch
        ...
        whiskContactTrial = LocalMinima(whiskTrial, 500, -0.1);  % detects peaks using LocalMinima (Buzsaki/Harris(kmf))
