function [stimTrig, whiskContactCell, whiskContacts, firstContactArr, firstContactArrAbsT ] = findWhiskerContactsAll()
...
stimEpochStart = stimTrig + sf/5;   % start looking for whisker contacts after motor artifact should have ended
stimEpochEnd = stimEpochStart + 2*sf;  % and stop just before the end motor artifact
...
%% select out epoch of whisker signal (based upon trial triggers)
    ...
    stdThresh = 3;  % stdev threshold for whisker touch detection
    ...
    if length(whiskContactTrial)>0
        whiskContactTrial = whiskContactTrial + sf/5;  % now adjusted for time since start of this trial
        ...
        firstContact = whiskContactTrial(1) + sf/5;  % just isolates the first whisker contact in the stimulus trial
...
% figure; hold on;
% for k = 1:length(firstContactArr)
%     plot(x(firstContactArrAbsT(k):(firstContactArrAbsT(k)+2*sf)));
% end
