function [correctRespStruc, animalCorrResp] = correctResp()

%% [correctRespStruc, cageCorrResp] = correctResp();
    ...
    if strfind(currDir(i).name, '.txt');    % make sure file is .txt, thus data
        ...
        for txtInd = 3:2:(length(fullCell)-5)
            ...
            if strfind(event1, 'bottom')
                ...
                if ~isempty(strfind(event1, 'top'))  || ~isempty(strfind(event1, 'catch'))
...
for numSess = 1:length(correctRespStruc)
    animalCorrResp(numSess) = length(correctRespStruc(numSess).correctStimTime)/length(correctRespStruc(numSess).corrRespArr);
end
