...
for i= 1:length(correctRespStruc)   % for all data files (animals or days)
    ...
    if length(stimTimeArr)>10    % check to make sure there are events in this file
        ...
        % calculate mean performance at end of session (last 10 bins)
        endOfSessionPerf = mean(binFracCorrect((length(binFracCorrect)-11):(length(binFracCorrect)-1)));
        binErrorRatesStruc(i).endOfSessionPerf = endOfSessionPerf;
