function [eventStruc] = detect16chEvents(nChannels, sf)
...
timeoutMs = 10;
timeout = timeoutMs*sf/1000;
thresh = -0.05;

% detect lever presses and lifts
levPress = LocalMinima(-dFiltLev, timeout, thresh);
levLift = LocalMinima(dFiltLev, timeout, thresh);

eventStruc.levPress = levPress;
eventStruc.levLift = levLift;

clear levSig filtLevSig dFiltLev;
...
% detect lever presses and lifts
rew = LocalMinima(-dRew, timeout, thresh);
...
MyFilt1=fir1(50,[10 200]/Nyquist);
filtWhiskSig = filtfilt(MyFilt1,1,whiskSig);

timeoutMs = 50;  % how many ms minimum between whisk touches
timeout = timeoutMs*sf/1000;    % figures out how many samples in timeout
thresh = -0.4;  % stdev threshold for whisker touch detection
    
whiskContacts = LocalMinima(-filtWhiskSig, timeout, thresh);  %  (-whiskSig, timeoutSamp, -(stdThresh*stdWhisk));
...
% detect lever presses and lifts
licks = LocalMinima(-dLick, timeout, thresh);
...
% thresh = -0.5;
% 
% frameTrig = LocalMinima(galvoSig, ;timeout, -0.5);
...
for k = 1:length(stimTrig)   % so for each stimulus trigger in the corresponding stimulus epoch
    ...
    timeoutMs = 50;  % how many ms minimum between whisk touches
    timeout = timeoutMs*sf/1000;    % figures out how many samples in timeout
    thresh = -0.1;  % stdev threshold for whisker touch detection
    
    whiskContactTrial = LocalMinima(-whiskTrial, timeout, thresh); 
