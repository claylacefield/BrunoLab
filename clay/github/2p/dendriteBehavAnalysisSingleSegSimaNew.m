function [dendriteBehavStruc, segStruc, frameAvgDf, eventStruc, x2] = dendriteBehavAnalysisSingleSegSimaNew(fps, toSave, toSeg, filterDate) % nChannels, fps, avg, roi)
...
sessionBasename = binFilename(1:strfind(binFilename, '.bin')-1);

%[eventStruc, x2] = detect2pEventsNameNew(sessionBasename, numFrames); % , numFrames);
[eventStruc, x2] = detect2pEventsSingleName(sessionBasename, numFrames);
...
for field = 1:length(fieldNames)
    ...
    if ~isempty(eventStruc.(fieldNames{field}))
        ...
        if (~isempty(strfind(fieldNames{field}, 'StimInd')) || ~isempty(strfind(fieldNames{field}, 'Time')))
            ...
            try
                hz=fps;
                [zeroFrame, eventCa , eventCaAvg, segEventCa, eventRoiHist, eventRoiHist2] = eventTrigCaRoiPk(filename, eventStruc, hz, segStruc, frameAvgDf, field);
                ...
                disp(['Problem in processing event: ' eventName]);
            end
