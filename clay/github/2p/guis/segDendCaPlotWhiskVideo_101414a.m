function varargout = segDendCaPlotWhiskVideo_101414a(varargin)
% SEGDENDCAPLOTWHISKVIDEO_101414A MATLAB code for segDendCaPlotWhiskVideo_101414a.fig
%      SEGDENDCAPLOTWHISKVIDEO_101414A, by itself, creates a new SEGDENDCAPLOTWHISKVIDEO_101414A or raises the existing
%      singleton*.
%
%      H = SEGDENDCAPLOTWHISKVIDEO_101414A returns the handle to a new SEGDENDCAPLOTWHISKVIDEO_101414A or the handle to
%      the existing singleton*.
%
%      SEGDENDCAPLOTWHISKVIDEO_101414A('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SEGDENDCAPLOTWHISKVIDEO_101414A.M with the given input arguments.
%
%      SEGDENDCAPLOTWHISKVIDEO_101414A('Property','Value',...) creates a new SEGDENDCAPLOTWHISKVIDEO_101414A or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before segDendCaPlotWhiskVideo_101414a_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to segDendCaPlotWhiskVideo_101414a_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help segDendCaPlotWhiskVideo_101414a

% Last Modified by GUIDE v2.5 14-Oct-2014 18:38:17

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @segDendCaPlotWhiskVideo_101414a_OpeningFcn, ...
                   'gui_OutputFcn',  @segDendCaPlotWhiskVideo_101414a_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before segDendCaPlotWhiskVideo_101414a is made visible.
function segDendCaPlotWhiskVideo_101414a_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to segDendCaPlotWhiskVideo_101414a (see VARARGIN)

% Choose default command line output for segDendCaPlotWhiskVideo_101414a
...
% UIWAIT makes segDendCaPlotWhiskVideo_101414a wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = segDendCaPlotWhiskVideo_101414a_OutputFcn(hObject, eventdata, handles) 
...
[filename, pathname] = uigetfile('.mat', 'Select segStruc .mat file');
cd(pathname);
...
% 101414 modifying for whisker video signals

fileBase = filename(1:(strfind(filename, '_seg')-1));

[whiskFilename, pathname] = uigetfile('.mat', 'Select whiskBehavStruc .mat file');
load(whiskFilename);
handles.whiskBehavStruc = whiskBehavStruc;
...
try
    ...
    for corrDirNum = 1:length(corrDir)
        if ~isempty(strfind(corrDir(corrDirNum).name, 'b.tif')) % && isempty(strfind(corrDir(corrDirNum).name, 'final'))
...
%     
%     event1 = eventHistBehavStruc.(eventFieldName1); % load in all trials for this event
%     event2 = eventHistBehavStruc.(eventFieldName2); % and the other event
    

    eventFieldName1 = [field1Base 'WhiskSig'];
    eventFieldName2 = [field2Base 'WhiskSig'];
    
    event1 = whiskBehavStruc.(eventFieldName1); % load in all trials for this event
    event2 = whiskBehavStruc.(eventFieldName2); % and the other event
    
        

    trialEvent1 = event1(:,trialSliderVal); % get single trial of event1
%     binSize = 100;
...
%     end
    ...
    axes(handles.axes5);
    %bar(trialEvent1);
    plot(trialEvent1); ylabel('whisker angle');
