...
save(['unitEpochCompilStruc_' mouseName '_' progTag '_' goodGreatSeg 'Seg_' date], 'unitEpochCompilStruc');
...
if toPlot
    try
        figure;
    subplot(2,1,1);
    scatter(unitEpochCompilStruc.unitRewEpochArr(:,1), unitEpochCompilStruc.unitRewEpochArr(:,2));
    hold on;
    line([0 0.1], [0 0.1], 'Color', 'r');
    title([mouseName ' ' goodGreatSeg 'Seg unitEpochScatter on ' date]);
    xlabel('Prestim epoch');
    ylabel('Poststim epoch 1');
