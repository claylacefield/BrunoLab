...
lockoutSec = 1; %2 (old paper?);
sdThresh = 2; %1 (old paper);%2; %1.5;
...
lockout = lockoutSec*frRate;%3*frRate; % 1000; 
%sdThresh = 1.5; %2; % 1;
wPks = threshold(ma2, sdThresh*std(ma2), lockout); %rv
...
% t = 1:length(ma);
% figure; plot(ma2); 
% hold on; plot(t(wPks), ma2(wPks), 'r.');
