...
if length(stimTimeArr) ~= length(stimTrig)
    if max(stim)>3 && length(stimTrig) < length(stimTimeArr)
        [correctRespStruc] = fixCorrRespStimInd(correctRespStruc, stimTrig);
        disp('Adjusting stimInds because some might have gotten cut off');
        stimTimeArr = correctRespStruc.stimTimeArr;
        stimTypeArr = correctRespStruc.stimTypeArr;
    else
