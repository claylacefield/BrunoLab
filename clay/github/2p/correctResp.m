function [correctRespStruc] = correctResp(varargin)


%% USAGE: [correctRespStruc] = correctResp(varargin);
%   080414: now general function for a variety of input situations;
%           SHOULD BE back-compatible with all previous versions

% 080414 made work for diff input configurations 
% (gui select, rowInd of dataFileArray, or a particular .txt filename)

if nargin == 0  % if no arguments, then gui select file
    [filename pathname] = uigetfile('*.txt', 'Select a text file of behavior to read');
    filepath = [pathname filename];
    cd(pathname);
elseif isnumeric(varargin{1})   % else if argument is a number, it's the rowInd
    rowInd = varargin{1};
...
else  % else if argument is string, then it's the .txt filename
    filename = varargin{1};
end
...
clear punRewToneSolSkipStimInd;
...
clear punRewToneSolSkipLatency;
...
% clear correctRewStimInd ...
%     correctStimInd corrLevPressLatency incorrectRewStimInd ...
%     incorrectStimInd incorrectUnrewStimInd incorrLevPressLatency ...
%     correctUnrewStimInd stimTypeArr stimTimeArr corrRespArr;
