function [datasetInfoStruc] = findDataFilenames(outStruc)
        ...
        if mouseDir(mouseDirInd).isdir
            ...
            for dayDirInd = 3:length(dayDir)
                ...
                if dayDir(dayDirInd).isdir
                    ...
                    for numDbs = 1:length(dbsInd)
                        
                        
                        try
                            load(sessionDir(dbsInd(numDbs)).name); %(latestDbsInd)).name);
                            rewVal1 = dendriteBehavStruc.rewStimStimIndCaAvg(1);
                            dataInd = find(rewCa1 == rewVal1);
                            
                            if ~isempty(dataInd)
                                sessionNameCell{dataInd} = sessionName;
                                mouseNameCell{dataInd} = mouseName;
                                dbsFilenameCell{dataInd} = sessionDir(dbsInd(numDbs)).name;
                                disp(['Found dataset ' sessionName ' for session #' num2str(dataInd)]);
                            end
                        catch
                        end
                    end % end FOR loop for all dendriteBehavStruc files
...
datasetInfoStruc.mouseNameCell = mouseNameCell;
datasetInfoStruc.sessionNameCell = sessionNameCell;
datasetInfoStruc.dbsFilenameCell = dbsFilenameCell;
