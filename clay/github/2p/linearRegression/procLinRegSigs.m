function [linRegSigStruc, frameAvgDf] = procLinRegSigs()
...
disp('Processing signals for linear regression'); tic;
...
% 062117 clay
% Adding whisker angle as another variable

whiskFilename = findLatestFilename('whiskDataStruc');
load(whiskFilename);
frTimes = whiskDataStruc.frTimes;
angle = whiskDataStruc.meanAngle(2:end);
angle = angle-mean(angle);
angle = -angle/std(angle);
...
for i = 1:nframes
    ...
    whiskAngle(i) = mean(angle(frTimes >= frameedge(i) & frTimes < frameedge(i+1)));
end
%keep lever discretized
leverb(leverb > 0) = 1;

linRegSigStruc.poleonsetb = poleonsetb;
linRegSigStruc.leverb = leverb;
linRegSigStruc.rewardb = rewardb;
linRegSigStruc.punb = punb;
linRegSigStruc.ncontact = ncontact;
linRegSigStruc.nlick = nlick;
linRegSigStruc.whiskAngle = whiskAngle;
