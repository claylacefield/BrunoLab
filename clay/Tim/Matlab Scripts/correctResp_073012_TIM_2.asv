function [correctRespStruc] = correctResp_073012_TIM_2
...
path =('\\10.112.43.36\Public\clay\mouse behavior\2012\010112');
...
for i = 3:length(currDir);
    ...
    if strfind(currDir(i).name, '.txt');    % make sure file is .txt, thus data
      ...
      startTag = strcmp(fullCell,'BEGIN DATA'); %starting at txt 'BEGIN DATA'
            beginInd = find(startTag)+1;
            
        for txtInd = beginInd:2:(length(fullCell)-2);
